...
public class ImportDataServiceImpl implements IImportDataService {
    ...
    public String insertMATable(MultipartFile excel) throws Exception {
        ...
        System.out.println("prex===="+prefix);
        String fileOtherName=fileName.substring(0, fileName.length()-prefix.length());//得到文件名。去掉了后缀
        System.out.println("fileOhterName"+fileOtherName);


        //限制上传文件的类型
        if (fileName.endsWith(".xls")) {
//            String path = "D://ESMSexcel/monthly_attendance_table/";
            String path = "/Users/suen//Desktop/monthly_attendance_table/";//文件会保存至指定路径
            File fileDirectory = new File(path);
            if (!fileDirectory.exists())
                fileDirectory.mkdirs();

            fileOtherName=fileOtherName+dateStr+prefix;
            File file = new File(path, fileOtherName);
            //完成上传
            try {
...
//            String path = "D://ESMSexcel/monthly_reissue_table/";
            String path = "/Users/suen//Desktop/monthly_reissue_table/";//文件会保存至指定路径
