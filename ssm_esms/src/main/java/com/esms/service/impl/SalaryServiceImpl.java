...
public class SalaryServiceImpl implements ISalaryService {
    ...
    public void insertSalaryByAcountAndDate(String eAccount, String date) throws CustomException {
        ...
        if (employee == null) {
            ...
            System.out.println("该员工此月是否有已经发放工资的工资项："+salaryMapper.selectByEidAndTimeAndStatus(employee.geteId(), date, 1));
            if (salaryMapper.selectByEidAndTimeAndStatus(employee.geteId(), date, 1) != null) {
                System.out.println("=========");
                throw new CustomException("本工号的此月工资已发放，不能再次结算");
    ...
    public void insertSalaryAllByDate(String date) throws CustomException {
        ...
        if (salaryList.size() >= 1) {
            throw new CustomException("此月已有员工工资发放,无法进行一键工资结算");
        } else {
     ...
     * @Return: int
     **/
    private void insertSalay(Employee employee, String date) throws CustomException {
        // 按照年月和工号查出这个员工的考勤记录，
        // 1.考勤存在就计算考勤
        // 2.考勤不存在即为0
        System.out.println("日期：  "+date);
        MonthlyAttendance monthlyAttendance = monthlyAttendanceMapper.selectByeIdAndDate(employee.geteId(), date);
        System.out.println("此月是否有考勤记录：  "+monthlyAttendance);

        // 判断数据库是否存在
        Salary salarySelect = salaryMapper.selectByEidAndTimeAndStatus(employee.geteId(), date, 0);
//        if (salarySelect != null){
//            throw new CustomException("本工号的此月工资已经结算，不能在结算");
//        }
//        System.out.println("salarySelect：  "+salarySelect);
...
//        salary.setdId(employee.getdId());   // 部门号
        ...
        System.out.println("dateNew为：  "+dateNew);
...
//            7
        if (monthlyAttendance != null) {
            ...
            double missionallowance = keyValueMapper.selectBykvKey("missionallowance").getKvValue();    //出差补贴
            ...
            //时薪  小时工资=月工资收入÷（月计薪天数×8小时）月计薪天数的计算方法为：（365天－104天）÷12月=21.75天（不扣除11天的法定节假日）
            ...
            // 加班补贴  平时加班1.5倍工资  周末加班2倍工资   节日加班3倍工资

            if (monthlyAttendance.getSickLeaveNum() > 3) { // 病假大于3天扣钱
                salary.setSickPay((monthlyAttendance.getSickLeaveNum() - 3) * (-30.00)); // 病假扣钱，一次三十
            } else {
                salary.setSickPay(0.00);
            }

            if (monthlyAttendance.getCompassionateLeaveNum() > 2) { // 事假大于2天扣钱
                salary.setThingPay((monthlyAttendance.getCompassionateLeaveNum() - 2) * (-50.00)); // 事假扣钱，一次50
            } else {
        ...
        int years = (int) (datesum / 31536000 / 1000);//一年31536000秒，再将毫秒化为秒
        try {
        ...
        double incomeTax = 0;
//        double taxable = shouldPay + insurances + attendance  - salary.getBusinessTravelPay() - 3500; // 纳税超额部分纳税超额部分
        double taxable = shouldPay + insurances + attendance - 3500; // 纳税超额部分纳税超额部分
